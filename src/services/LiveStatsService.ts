...
function extractTeam(raw: any, side: "home" | "away", stats: any): TeamLiveStats {
 ...
 */
export const getMatchLiveStats = async (eventId: string): Promise<MatchLiveStats | null> => {
    try {
        const raw = await getLiveEventById(eventId);
        if (!raw) {
