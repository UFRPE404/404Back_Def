...
type PoissonLike = {
    eventDistribution: (lambda: number) => DistributionEntry[];
...
export class PlayerEngine {
     ...
     */
    private extractPlayerStats(
        events: PlayerMatchEvent[],
        playerId: string,
    ): PlayerStatRates & { avgMinutes: number; count: number } {
        ...
        const sorted = [...playerEvents].sort(
            (a, b) => Number(a.event.time) - Number(b.event.time),
        );

        const stats = {
            shots: [] as number[],
            shots_on_goal: [] as number[],
            yellowcard: [] as number[],
            redcard: [] as number[],
            corners: [] as number[],
            goals: [] as number[],
            minutes: [] as number[],
        ...
        const avgMinutes =
            stats.minutes.reduce((a, b) => a + b, 0) / stats.minutes.length;
     ...
     */
    private applyPlayerContext(
        rates: PlayerStatRates,
        ...
        const attackMultiplier =
            (context.isOffensivePlayer ? 1.2 : 1.0) *
    ...
    analyzePlayer(
        ...
        return {
            ...
            distributions: {
                ...
                shots_on_goal: this.poisson.eventDistribution(
                    lambdas.shots_on_goal,
                ),
