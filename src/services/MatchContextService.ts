...
type EventKey = keyof PlayerAnalysisResult["distributions"];
...
export class MatchContextService {
     ...
     */
    minuteFactor(minute: number): number {
     ...
     */
    scoreFactor(event: EventKey, scoreDiff: number, minute: number): number {
        const isLate = minute >= 60;

        if (event === "shots" || event === "shots_on_goal" || event === "goals") {
            // Perdendo → ataca mais; ganhando → recua
            if (scoreDiff < 0) return isLate ? 1.3 : 1.15;
            if (scoreDiff > 0) return isLate ? 0.8 : 0.9;
            return 1.0;
        }

        if (event === "yellowcard" || event === "redcard") {
            // Jogos apertados e finais tendem a ter mais cartões
            if (scoreDiff === 0 && isLate) return 1.2;
            if (Math.abs(scoreDiff) >= 2) return 0.85; // jogo resolvido
            return 1.0;
        }

        if (event === "corners") {
            if (scoreDiff < 0) return isLate ? 1.25 : 1.1;
            if (scoreDiff > 0) return 0.9;
            return 1.0;
        }
