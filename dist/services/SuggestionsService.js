...
exports.getFeaturedMatchIds = getFeaturedMatchIds;
exports.getSuggestions = getSuggestions;
exports.getBestOfDay = getBestOfDay;
exports.getDreamBets = getDreamBets;
...
function isTargetLeague(league) {
...
function parseScore(event) {
    ...
    if (ss && typeof ss === "string" && ss.includes("-")) {
        const parts = ss.split("-").map((s) => parseInt(s.trim(), 10));
        const h = parts[0] ?? NaN;
        const a = parts[1] ?? NaN;
        if (!isNaN(h) && !isNaN(a))
...
async function getTeamContext(teamId, teamName, isHome) {
    const results = [];
    const goalsScored = [];
    const goalsConceded = [];
    let cleanSheets = 0;
    try {
        ...
        const page1 = await (0, betsApiService_1.getTeamHistory)(teamId, 1);
        let matches = page1?.results ?? [];
        if (matches.length < 10) {
            try {
                const page2 = await (0, betsApiService_1.getTeamHistory)(teamId, 2);
...
async function fetchEnrichedFromAnyLeague(allMatches) {
    ...
    if (withOdds.length === 0) {
        ...
        for (const match of candidates) {
            try {
                const { simpleOdds } = await (0, MatchService_1.getOddsForMatch)(String(match.id));
                if (simpleOdds && simpleOdds[0] !== null) {
                    enrichedAny.push({ ...match, simpleOdds });
                }
            }
            catch { /* skip */ }
            if (enrichedAny.length >= 10)
                break;
        }
        if (enrichedAny.length === 0)
            return [];
        // Usa esses como pool
...
async function enrichFromPool(pool) {
    const enriched = [];
...
function buildMatchSummaryForAI(match, index) {
    ...
    if (h.recentResults.length > 0) {
        ...
        if (hWinStreak >= 3)
            summary += ` | ${hWinStreak} vitórias seguidas!`;
        summary += "\n";
    }
    if (a.recentResults.length > 0) {
        ...
        if (aWinStreak >= 3)
            summary += ` | ${aWinStreak} vitórias seguidas!`;
        summary += "\n";
    }
    return summary;
}
async function generateAIAnalysis(matches) {
...
[
  {
    ...
    try {
        ...
        if (!jsonMatch)
            return [];
        const aiPicks = JSON.parse(jsonMatch[0]);
        return aiPicks
            ...
            if (pickLower.includes("empate"))
                pickOdd = drawOdd;
            else if (pickLower.includes(match.away.toLowerCase()))
                pickOdd = awayOdd;
            else if (pickLower.includes("over"))
                pickOdd = homeOdd; // placeholder
            else if (pickLower.includes("under"))
                pickOdd = drawOdd; // placeholder
            else if (pickLower.includes("ambos"))
                pickOdd = drawOdd; // placeholder
            return {
                id: `${p.type}-${match.id}`,
                matchId: match.id,
...
function generateHeuristicSuggestions(matches) {
    const suggestions = [];
    for (const match of matches) {
        ...
        if (homeProb > 0.50 && homeOdd >= 1.25) {
            ...
            if (hStreak >= 2)
                parts.push(`vem de ${hStreak} vitórias seguidas`);
            if (h.avgGoalsScored > 0)
                parts.push(`marca em média ${h.avgGoalsScored} gols/jogo`);
            if (a.avgGoalsConceded > 1)
                parts.push(`${match.away} sofre ${a.avgGoalsConceded} gols/jogo fora de casa`);
            suggestions.push({
                id: `best-${match.id}`,
                matchId: match.id,
                teamA: match.home,
            ...
            if (aStreak >= 2)
                parts.push(`com ${aStreak} vitórias seguidas`);
            if (a.avgGoalsScored > 0)
                parts.push(`média de ${a.avgGoalsScored} gols/jogo`);
            suggestions.push({
                id: `best-${match.id}`,
                matchId: match.id,
                teamA: match.home,
...
// ─── Featured Matches ───────────────────
async function selectFeaturedWithAI(matches) {
...
async function getFeaturedMatchIds() {
    ...
    try {
        const { matches: allMatches } = await (0, MatchService_1.getMatchesWithOdds)();
        if (GROQ_API_KEY) {
 ...
 */
async function fetchNextDayMatches() {
    for (let offset = 1; offset <= 3; offset++) {
        const dayStr = getDayStr(offset);
        console.log(`[Suggestions] Buscando jogos do dia ${dayStr} (hoje +${offset})...`);
        try {
            const rawGames = await (0, betsApiService_1.getAllUpcomingForDay)(dayStr);
            const filtered = rawGames.filter((g) => !isVirtualMatch(g));
...
// ─── Função principal ───────────────────
async function getSuggestions(day) {
    ...
    try {
        ...
        let allMatches;
        if (day) {
            const rawGames = await (0, betsApiService_1.getAllUpcomingForDay)(day);
            allMatches = rawGames
                .filter((g) => !isVirtualMatch(g))
                .map(mapRawGameToMatch);
            console.log(`[Suggestions] ${allMatches.length} jogos encontrados para o dia ${day}`);
        }
        else {
            let { matches } = await (0, MatchService_1.getMatchesWithOdds)();
            allMatches = matches;
